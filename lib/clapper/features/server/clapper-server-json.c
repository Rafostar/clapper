...
#define _ADD_KEY_VAL_FLOAT(key, val)                           \
    _JSON_AUTO_COMMA                                           \
    g_string_append_printf (_json, "\"%s\":%.2f", key, (gfloat) val);
    ...
    guint played_index, GPtrArray *items)
{
  ...
  CLAPPER_SERVER_JSON_BUILD (&data, {
    ...
    _ADD_KEY_VAL_FLOAT ("speed", clapper_player_get_speed (player));
    _ADD_KEY_VAL_FLOAT ("volume", clapper_player_get_volume (player));
      ...
      _ADD_NAMED_ARRAY ("items", {
        ...
        for (i = 0; i < items->len; ++i) {
          _ADD_OBJECT ({
            ...
            /* TODO: Add more info per item (including available/selected tracks) */
          });
        }
      });
    });
