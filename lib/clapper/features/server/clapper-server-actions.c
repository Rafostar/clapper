...
static gboolean
_parse_float (const gchar *text, gfloat *val)
{
  if (!_string_is_number (text, TRUE))
    return FALSE;

  *val = (gfloat) g_ascii_strtod (text, NULL);
...
_parse_boolean (const gchar *text, gboolean *val)
{
  ...
  if ((res = (g_strcmp0 (text, "true") == 0)))
    *val = TRUE;
  else if ((res = (g_strcmp0 (text, "false") == 0)))
    *val = FALSE;
...
inline gboolean
clapper_server_actions_parse_seek (const gchar *text, gfloat *position)
{
  /* "seek" + whitespace = 5 */
  if (!_parse_float (text + 5, position))
    return FALSE;
...
inline gboolean
clapper_server_actions_parse_set_speed (const gchar *text, gfloat *speed)
{
  /* "set_speed" + whitespace = 10 */
  return _parse_float (text + 10, speed);
}

inline gboolean
clapper_server_actions_parse_set_volume (const gchar *text, gfloat *volume)
{
  /* "set_volume" + whitespace = 11 */
  if (!_parse_float (text + 11, volume))
    return FALSE;
...
clapper_server_actions_parse_set_progression (const gchar *text, ClapperQueueProgressionMode *mode)
{
  ...
  if ((res = (g_strcmp0 (text, CLAPPER_SERVER_QUEUE_PROGRESSION_CONSECUTIVE) == 0)))
    *mode = CLAPPER_QUEUE_PROGRESSION_CONSECUTIVE;
  else if ((res = (g_strcmp0 (text, CLAPPER_SERVER_QUEUE_PROGRESSION_REPEAT_ITEM) == 0)))
    *mode = CLAPPER_QUEUE_PROGRESSION_REPEAT_ITEM;
  else if ((res = (g_strcmp0 (text, CLAPPER_SERVER_QUEUE_PROGRESSION_CAROUSEL) == 0)))
    *mode = CLAPPER_QUEUE_PROGRESSION_CAROUSEL;
  else if ((res = (g_strcmp0 (text, CLAPPER_SERVER_QUEUE_PROGRESSION_SHUFFLE) == 0)))
...
clapper_server_actions_parse_add (const gchar *text, const gchar **uri)
{
  ...
  /* FIXME: Use shared Clapper URI validation function */
  if (!g_uri_is_valid (text, G_URI_FLAGS_ENCODED, NULL))
    return FALSE;
...
clapper_server_actions_parse_insert (const gchar *text, gchar **uri, guint *after_id)
{
  ...
  if ((res = (g_strv_length (data) == 2
      && g_uri_is_valid (data[0], G_URI_FLAGS_ENCODED, NULL) // FIXME: Use shared Clapper URI validation function
