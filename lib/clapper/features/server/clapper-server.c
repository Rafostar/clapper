...
#define FLOAT_EVENT_MAX_SIZE 12
...
static inline void
clapper_server_send_ws_float_event (ClapperServer *self, const gchar *event, gfloat val)
{
  gchar text[FLOAT_EVENT_MAX_SIZE];

  g_snprintf (text, sizeof (text), "%s %.2f", event, val);
...
static void
clapper_server_position_changed (ClapperFeature *feature, gfloat position)
{
  ClapperServer *self = CLAPPER_SERVER_CAST (feature);
...
static void
clapper_server_speed_changed (ClapperFeature *feature, gfloat speed)
{
  ClapperServer *self = CLAPPER_SERVER_CAST (feature);
  ...
  if (self->running && self->ws_connections->len > 0)
    clapper_server_send_ws_float_event (self, CLAPPER_SERVER_WS_EVENT_SPEED, speed);
}

static void
clapper_server_volume_changed (ClapperFeature *feature, gfloat volume)
{
  ClapperServer *self = CLAPPER_SERVER_CAST (feature);
  ...
  if (self->running && self->ws_connections->len > 0)
    clapper_server_send_ws_float_event (self, CLAPPER_SERVER_WS_EVENT_VOLUME, volume);
}
