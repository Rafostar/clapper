...
_get_next_item_unlocked (ClapperQueue *self, ClapperQueueProgressionMode mode)
{
  ...
  switch (mode) {
    ...
    case CLAPPER_QUEUE_PROGRESSION_SHUFFLE:{
      ...
      guint random_index = g_rand_int_range (rand, 0, self->items->len);

      next_item = g_ptr_array_index (self->items, random_index);
...
gboolean
clapper_queue_handle_eos (ClapperQueue *self)
{
  ClapperMediaItem *next_item = NULL;
  ClapperQueueProgressionMode progression_mode;
  ...
  if ((next_item = _get_next_item_unlocked (self, progression_mode))) {
    handled_eos = clapper_queue_select_item (self, next_item);
    gst_object_unref (next_item);
  }
  CLAPPER_QUEUE_REC_UNLOCK (self);
...
clapper_queue_reposition_item (ClapperQueue *self, ClapperMediaItem *item, gint index)
{
  ...
  if (g_ptr_array_find (self->items, item, &index_old)) {
    ...
    guint start_index, end_index, n_changed;

    if (index == -1)
      index = self->items->len - 1;

    GST_DEBUG_OBJECT (self, "Reposition item %u -> %i, is_current: %s",
        index_old, index, (item == self->current_item) ? "yes" : "no");

    removed_item = g_ptr_array_steal_index (self->items, index_old);
    g_ptr_array_insert (self->items, index, removed_item);

    _announce_reposition (self, index_old, index);

    if (self->current_index != CLAPPER_QUEUE_INVALID_POSITION) {
      guint before = self->current_index;

      if (index_old > self->current_index && index <= self->current_index)
        self->current_index++; // Moved before current item
      else if (index_old < self->current_index && index >= self->current_index)
        self->current_index--; // Moved after current item
      else if (index_old == self->current_index)
        self->current_index = index; // Moved current item

      if (self->current_index != before)
        _announce_current_index_change (self);
    }

    start_index = MIN (index_old, index);
    end_index = MAX (index_old, index);
 ...
 * @queue: a #ClapperQueue
 * @index: an item index or #CLAPPER_QUEUE_INVALID_POSITION to unselect
 *
 * Selects #ClapperMediaItem at @index from @queue as current one or
 * unselects currently selected index when @index is #CLAPPER_QUEUE_INVALID_POSITION.
 *
...
_select_item_with_offset (ClapperQueue *self, gint offset)
{
  ...
  if (self->current_index != CLAPPER_QUEUE_INVALID_POSITION
      && self->current_index + offset >= 0)
    success = clapper_queue_select_index (self, self->current_index + offset);
 ...
 *
 * Returns: Current item index or #CLAPPER_QUEUE_INVALID_POSITION
