 ...
 *
 * #ClapperGtkVideo is the main widget exposed by `ClapperGtk` API. All other widgets
 ...
 * nested within another widget like [class@Gtk.Box]) automatically controls
 ...
 * widgets such as [class@ClapperGtk.SimpleControls] are also available.
 *
 * # Basic usage
 *
...
static void
_mark_active_cb (ClapperGtkVideo *self)
{
  GST_DEBUG_OBJECT (self, "Window is now active");
...
{
  g_clear_handle_id (&self->active_timeout, g_source_remove);

  /* We deactivate immediately */
  if (!gtk_window_is_active (window)) {
    GST_DEBUG_OBJECT (self, "Window is now inactive");
    self->is_active = FALSE;

    return;
  }

  /* XXX: We have a delay to marking as active on idle so user can
   * click on a window without triggering toggle play action */
  GST_DEBUG_OBJECT (self, "Window is activating...");
  self->active_timeout = g_idle_add_once ((GSourceOnceFunc) _mark_active_cb, self);
}
...
_determine_can_fade (ClapperGtkVideo *self)
{
  ...
  if (!self->is_touch && self->x > 0 && self->y > 0) {
    GtkWidget *tmp_widget = gtk_widget_pick (video, self->x, self->y, GTK_PICK_DEFAULT);
    guint i;
...
_handle_motion (ClapperGtkVideo *self, GtkEventController *controller, gdouble x, gdouble y)
{
  ...
  /* FIXME: Have some motion threshold, but make it not
   * interfere with last points comparison */
...
static void
_click_delay_cb (ClapperGtkVideo *self)
{
  GST_LOG_OBJECT (self, "Delayed click handler reached");
  self->click_timeout = 0;

  switch (clapper_player_get_state (self->player)) {
    ...
    gdouble x, gdouble y, ClapperGtkVideo *self)
{
  ...
  /* Prevent single click from toggling play when window is not an active one.
   * In this case we just want to bring window to the top and resume interaction. */
  if (!self->is_active && n_press == 1) {
    GST_DEBUG_OBJECT (self, "Ignoring left click on inactive window");
  ...
  switch (gdk_device_get_source (device)) {
    ...
    default:
      if (!_is_on_leading_overlay (self, CLAPPER_GTK_VIDEO_ACTION_TOGGLE_PLAY)) {
        self->click_timeout = g_timeout_add_once (320, (GSourceOnceFunc) _click_delay_cb, self);
        gtk_gesture_set_state (GTK_GESTURE (click), GTK_EVENT_SEQUENCE_CLAIMED);
      }
      break;
  }
...
{
  if (!_is_on_leading_overlay (self, CLAPPER_GTK_VIDEO_ACTION_TOGGLE_FULLSCREEN)) {
    ...
    gdouble x, gdouble y, ClapperGtkVideo *self)
{
  ...
  g_clear_handle_id (&self->click_timeout, g_source_remove);
...
_ensure_css_provider (void)
{
  ...
  if (G_LIKELY (display != NULL)) {
    ...
    gtk_css_provider_load_from_resource (provider,
        "/com/github/rafostar/Clapper/clapper-gtk/css/styles.css");
...
_set_buffering_animation_enabled (ClapperGtkVideo *self, gboolean enabled)
{
  ...
  gtk_widget_set_visible (GTK_WIDGET (self->buffering_animation), enabled);
 ...
 *
 * Returns: (transfer full): a new video #GtkWidget.
 */
GtkWidget *
...
clapper_gtk_video_class_init (ClapperGtkVideoClass *klass)
{
  ...
  gtk_widget_class_set_template_from_resource (widget_class,
      "/com/github/rafostar/Clapper/clapper-gtk/ui/clapper-gtk-video.ui");
