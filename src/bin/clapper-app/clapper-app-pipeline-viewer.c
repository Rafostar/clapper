    ...
    gdouble dx, gdouble dy, ClapperAppPipelineViewer *self)
{
  ...
  if (clapper_app_pipeline_viewer_focus (self,
      &GRAPHENE_RECT_INIT (x, y, w, h), calc_scale)) {
    /* Allow setting higher value than current upper, this will be
     * recalculated again in size allocate before draw anyway */
...
_refresh_viewport (ClapperAppPipelineViewer *self)
{
  ...
  clapper_app_pipeline_viewer_focus (self,
      &GRAPHENE_RECT_INIT (x, y, w, h), self->zoom);
...
{
  /* Show preview image again since our viewport changed */
  clapper_app_pipeline_viewer_invalidate_viewport (self);

  /* Wait a bit while adjustment still moves, then refresh */
  g_clear_handle_id (&self->refresh_id, g_source_remove);
  self->refresh_id = g_timeout_add_once (500, (GSourceOnceFunc) _refresh_viewport, self);
}
...
gboolean
clapper_app_pipeline_viewer_focus (ClapperAppPipelineViewer *self,
    const graphene_rect_t *viewport, gdouble zoom)
{
  /* Need to wait until previous zoom finishes. Otherwise scrollbars values
   * would be out of sync. We also await preview texture before zoom can work. */
  if (!self->preview_texture)
    return FALSE;

  graphene_rect_init_from_rect (&self->viewport, viewport);
...
void
clapper_app_pipeline_viewer_invalidate_viewport (ClapperAppPipelineViewer *self)
{
...
gdouble
clapper_app_pipeline_viewer_get_zoom (ClapperAppPipelineViewer *self)
{
  return self->zoom;
}
