...
static gchar *
window_title_closure (ClapperAppWindow *self, ClapperMediaItem *current_item)
{
  gchar *title = NULL;

  if (current_item)
    title = clapper_media_item_get_title (current_item);

  /* Use media item title for app window title when something
   * is playing, otherwise show application name */
  return (title) ? title : g_strdup (CLAPPER_APP_NAME);
}
    ...
    gdouble x, gdouble y, ClapperAppWindow *self)
{
  ...
  if (gdk_toplevel_show_window_menu (GDK_TOPLEVEL (surface), event)) {
    GdkCursor *cursor = gtk_widget_get_cursor (self->video);
    ...
    gdouble x, gdouble y, ClapperAppWindow *self)
{
  ...
  width = gtk_widget_get_width (self->video);
  area_w = (width / 6.);

  if (x <= area_w || (forward = (x >= width - area_w))) {
    _handle_seek_key_press (self, forward);

    /* We are here after video click handler, so pause delay
     * timeout is now cleared and after claiming event from
     * second click it will not be set anymore */
    gtk_gesture_set_state (GTK_GESTURE (click), GTK_EVENT_SEQUENCE_CLAIMED);
  }
}

static void
touch_stopped_cb (GtkGestureClick *click, ClapperAppWindow *self)
{
  if (self->seeking)
    _end_seek_operation (self);
}
...
clapper_app_window_init (ClapperAppWindow *self)
{
  ...
  g_object_set (settings, "gtk-double-click-distance", 32, NULL);
...
clapper_app_window_class_init (ClapperAppWindowClass *klass)
{
  ...
  gtk_widget_class_bind_template_callback (widget_class, window_title_closure);
  ...
  gtk_widget_class_bind_template_callback (widget_class, touch_stopped_cb);
