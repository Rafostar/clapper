...
#include "../clapper-tube-extractor-private.h"
...
ClapperTubeHarvest *
clapper_tube_src_run_extraction (ClapperTubeSrc *self, const gchar *uri,
    GCancellable *cancellable, GError **error)
{
  ...
  if (!(guri = g_uri_parse (uri, G_URI_FLAGS_ENCODED, error)))
    return NULL;
  ...
  if (!(extractor = clapper_tube_loader_get_extractor_for_uri (guri))) {
    ...
    GST_ERROR ("No plugin for URI: %s", latest_uri);
  ...
  harvest = clapper_tube_extractor_run (extractor, guri, cancellable, error);
