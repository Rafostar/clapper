...
static gboolean
clapper_playlistable_default_parse (ClapperPlaylistable *self, GBytes *bytes,
...
gboolean
clapper_playlistable_parse (ClapperPlaylistable *self, GBytes *bytes,
    GListStore *playlist, GCancellable *cancellable, GError **error)
{
  ClapperPlaylistableInterface *iface = CLAPPER_PLAYLISTABLE_GET_IFACE (self);

  return iface->parse (self, bytes, playlist, cancellable, error);
}
