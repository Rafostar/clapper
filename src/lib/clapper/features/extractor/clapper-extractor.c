...
clapper_extractor_queue_item_added (ClapperFeature *feature, ClapperMediaItem *item, guint index)
{
  ...
  /* Only create director and let it handle item if URI is supported */
  if (G_LIKELY (uri != NULL)) {
    if (clapper_addons_loader_has_extension_for_uri (uri, CLAPPER_TYPE_EXTRACTABLE)) {
      if (!self->director)
        self->director = clapper_extractor_director_new ();

      clapper_extractor_director_enqueue (self->director, item);
    }
    g_uri_unref (uri);
  }
}
...
clapper_extractor_queue_item_removed (ClapperFeature *feature, ClapperMediaItem *item, guint index)
{
  ...
  if (G_LIKELY (self->director != NULL))
    clapper_extractor_director_dequeue (self->director, item);
}
...
clapper_extractor_queue_cleared (ClapperFeature *feature)
{
  ...
  if (G_LIKELY (self->director != NULL))
    clapper_extractor_director_dequeue_all (self->director);
}
...
clapper_extractor_unprepare (ClapperFeature *feature)
{
  ...
  clapper_extractor_queue_cleared (feature);

  gst_clear_object (&self->director);
