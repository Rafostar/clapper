 ...
 * for applications that use toolkits such as #GTK. You can do this manually or use
 * provided reactable convenience functions.
 *
 * Due to the threaded nature, you should also avoid comparing to the current properties
 * values in the player or its queue. While these are thread safe, there is no guarantee
 * that values/objects between threads are still the same in both (or still exist).
 * Monitor current values by implementing this interface virtual functions instead.
 * These are all serialized into your implementation thread.
 *
 * Returns: (transfer full) (nullable): A reference to the #ClapperPlayer.
 *
 * Since: 0.10
 */
ClapperPlayer *
    ...
    ClapperMediaItem *item, ClapperMediaItem *after_item)
{
  ...
  g_return_if_fail (CLAPPER_IS_REACTABLE (self));
  g_return_if_fail (CLAPPER_IS_MEDIA_ITEM (item));
  g_return_if_fail (CLAPPER_IS_MEDIA_ITEM (after_item) || after_item == NULL);
