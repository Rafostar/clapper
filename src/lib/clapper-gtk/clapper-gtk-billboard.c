...
_player_volume_changed_cb (ClapperPlayer *player,
    GParamSpec *pspec G_GNUC_UNUSED, ClapperGtkBillboard *self)
{
  self->volume = PERCENTAGE_ROUND (clapper_player_get_volume (player));
...
_player_speed_changed_cb (ClapperPlayer *player,
    GParamSpec *pspec G_GNUC_UNUSED, ClapperGtkBillboard *self)
{
  self->speed = PERCENTAGE_ROUND (clapper_player_get_speed (player));

  clapper_gtk_billboard_announce_speed (self);
}
...
clapper_gtk_billboard_announce_volume (ClapperGtkBillboard *self)
{
  ...
  percent_str = g_strdup_printf ("%.0lf%%", self->volume * 100);

  if (self->volume <= 1.0) {
    gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (self->top_progress), 0.0);
    gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (self->bottom_progress), self->volume);

    if (has_overamp)
      gtk_widget_remove_css_class (self->progress_box, "overamp");
  } else {
    gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (self->top_progress), self->volume - 1.0);
  ...
  gtk_image_set_from_icon_name (GTK_IMAGE (self->progress_image),
      clapper_gtk_get_icon_name_for_volume ((!self->mute) ? self->volume : 0));
...
clapper_gtk_billboard_announce_speed (ClapperGtkBillboard *self)
{
  ...
  speed_str = g_strdup_printf ("%.2lfx", self->speed);

  if (self->speed <= 1.0) {
    gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (self->top_progress), 0.0);
    gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (self->bottom_progress), 1.0 - self->speed);
  } else {
    gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (self->top_progress), self->speed - 1.0);
    gtk_progress_bar_set_fraction (GTK_PROGRESS_BAR (self->bottom_progress), 0.0);
  }

  gtk_image_set_from_icon_name (GTK_IMAGE (self->progress_image),
      clapper_gtk_get_icon_name_for_speed (self->speed));
...
clapper_gtk_billboard_root (GtkWidget *widget)
{
  ...
  if ((player = clapper_gtk_get_player_from_ancestor (widget))) {
    ...
    self->volume = clapper_player_get_volume (player);
    self->mute = clapper_player_get_mute (player);
    self->speed = clapper_player_get_speed (player);
  }
}
