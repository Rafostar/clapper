    ...
    gdouble x, gdouble y, ClapperAppQueueList *self)
{
  ...
  if (G_VALUE_HOLDS (value, GTK_TYPE_WIDGET)) {
    ...
    ClapperMediaItem *new_item;

    if ((new_item = clapper_app_utils_media_item_from_value (value))) {
      clapper_queue_insert_item (queue, new_item, drop_index);
      success = TRUE;
...
static void
_item_selected_cb (ClapperGtkQueueSelection *selection, guint index, ClapperAppQueueList *self)
{
...
GtkWidget *
clapper_app_queue_list_new (void)
{
  return g_object_new (CLAPPER_APP_TYPE_QUEUE_LIST, NULL);
}
...
clapper_app_queue_list_realize (GtkWidget *widget)
{
  ...
  if ((player = clapper_gtk_get_player_from_ancestor (widget))) {
    ...
    ClapperGtkQueueSelection *selection = clapper_gtk_queue_selection_new (queue);
...
clapper_app_queue_list_class_init (ClapperAppQueueListClass *klass)
{
  ...
  gtk_widget_class_set_template_from_resource (widget_class,
      "/com/github/rafostar/Clapper/clapper-app/ui/clapper-app-queue-list.ui");
